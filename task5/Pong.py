...
class FuzzyPlayer(Player):
    def __init__(self, racket: Racket, ball: Ball, board: Board):
        ...
        x_dist = fuzzcontrol.Antecedent(np.arange(-200, 200, 1), "x_dist")
        x_dist.automf(5, "quantity", ["far_left", "left", "center", "right", "far_right"])

        y_dist = fuzz.control.Antecedent(np.arange(0, 350, 1), "y_dist")
        y_dist.automf(3, "quantity", ["close", "medium", "far"])

        speed = fuzz.control.Consequent(np.arange(-50, 50, 1), "speed")
        speed.automf(5, "quantity", ["fast_left", "left", "stay", "right", "fast_right"])

        # ruleset = [
        #     fuzzcontrol.Rule(x_dist["far_left"] | x_dist["left"], speed["fast_left"]),
        #     fuzzcontrol.Rule(x_dist["center"], speed["stay"]),
        #     fuzzcontrol.Rule(x_dist["far_right"] | x_dist["right"], speed["fast_right"])
        # ]

        ruleset = [
            fuzzcontrol.Rule(y_dist["close"] & x_dist["far_left"], speed["fast_left"]),
            fuzzcontrol.Rule(y_dist["close"] & x_dist["left"], speed["fast_left"]),
            fuzzcontrol.Rule(y_dist["close"] & x_dist["center"], speed["stay"]),
            fuzzcontrol.Rule(y_dist["close"] & x_dist["right"], speed["fast_right"]),
            fuzzcontrol.Rule(y_dist["close"] & x_dist["far_right"], speed["fast_right"]),

            fuzzcontrol.Rule(y_dist["medium"] & x_dist["far_left"], speed["fast_left"]),
            fuzzcontrol.Rule(y_dist["medium"] & x_dist["left"], speed["left"]),
            fuzzcontrol.Rule(y_dist["medium"] & x_dist["center"], speed["stay"]),
            fuzzcontrol.Rule(y_dist["medium"] & x_dist["right"], speed["right"]),
            fuzzcontrol.Rule(y_dist["medium"] & x_dist["far_right"], speed["fast_right"]),

            fuzzcontrol.Rule(y_dist["far"] & x_dist["far_left"], speed["left"]),
            fuzzcontrol.Rule(y_dist["far"] & x_dist["left"], speed["left"]),
            fuzzcontrol.Rule(y_dist["far"] & x_dist["center"], speed["stay"]),
            fuzzcontrol.Rule(y_dist["far"] & x_dist["right"], speed["right"]),
            fuzzcontrol.Rule(y_dist["far"] & x_dist["far_right"], speed["right"]),
        ]

        self.racket_controller = fuzz.control.ControlSystem(ruleset)

    def act(self, x_diff: int, y_diff: int):
        velocity = self.make_decision(x_diff, y_diff)
        self.move(self.racket.rect.center[0]-40+velocity)

    def make_decision(self, x_diff: int, y_diff: int):
        speed = fuzzcontrol.ControlSystemSimulation(self.racket_controller)
        speed.input["x_dist"] = -x_diff
        speed.input["y_dist"] = -y_diff
